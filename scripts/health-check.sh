...
# Check embedding progress - get count from last 2 hours (allows for slow summarization)
RECENT_EMBEDDINGS=$(docker exec $POSTGRES_CONTAINER psql -U mindmeld -d conversations -t -c "
SELECT COUNT(*) FROM embeddings WHERE created_at > NOW() - INTERVAL '2 hours'
" 2>/dev/null | tr -d ' ')
...
if [[ -n "$CONTAINER_UPTIME" ]] && [[ "$RECENT_EMBEDDINGS" -lt 50 ]]; then
    ...
    if [[ "$PENDING" -gt 1000 ]]; then
        osascript -e "display notification \"Embedding stalled! $PENDING pending, only $RECENT_EMBEDDINGS in last 2 hours\" with title \"Mindmeld Alert\" sound name \"Basso\""
        exit 1
    fi
fi
...
echo "Health check passed - $RECENT_EMBEDDINGS embeddings in last 2 hours"
