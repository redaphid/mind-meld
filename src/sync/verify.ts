    ...
    for (const sessionFile of discovered.files) {
      try {
        ...
        result.drifted.push({
          ...
          // Canonical form when a cwd exists; the raw directory name — never
          // the lossy decode — when it does not (#33).
          projectPath: canonicalizeProjectPath(parsed.cwd) ?? projectDirName,
          counts,
          drift,
          repaired,
        });
