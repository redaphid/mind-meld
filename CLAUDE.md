...
## Standing ingestion down (getting the GPU back)

Embedding and summarization hold an Ollama slot for minutes at a time. When the
same GPU is wanted for something else — a game, a ComfyUI render — the overview
screen has a **Stop until next cycle** button next to *Run ingestion now*, and
the same thing is `POST /api/pause` (`DELETE /api/pause` to resume immediately).

It is deliberately **not** a pause switch:

- The signal is a short deadline in `sync_control` (`init-db/022-sync-pause.sql`).
  The queue does not run in the process serving the UI — it runs in the sync
  workers, in their own containers — so Postgres is the only channel between the
  button and the work.
- Workers act on it at their next checkpoint: between embedding batches, before
  each long-message summarization, and between summarized sessions. Whatever was
  in flight is left pending, never half-written. An LLM call already issued
  finishes first, so the stop lands in seconds to about a minute, not instantly.
- The window is minutes (default 15, clamped 1–240) against an interval of an
  hour, so **the next scheduled cycle runs as normal** with nobody having to
  switch anything back on. The worst case of a forgotten press, or a worker
  killed mid-pass, is one skipped cycle — it cannot leave the index off.
- A stood-down cycle exits **0**, not 1. It is a button someone pressed, not a
  failed run, and the container loop must not page anyone for it. The run summary
  says `Stood down` so a thin cycle still explains itself.
- The dashboard reports the queue as `standing-down`, which outranks every
  rate-derived verdict — same class of fix as #109/#111: a queue stopped on
  purpose must never read as `stalled`.

Reading `sync_control` **fails open**. Migrations are applied by the `mcp`
service on startup, so a sync worker can legitimately meet a database that has
not got the table yet; a switch that cannot be read is not a switch that says
stop.
...
with `ON CONFLICT DO UPDATE` — the chunk path just lacks it.
